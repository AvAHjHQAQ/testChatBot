...
class VoiceDialogSystem:
    ...
    def __init__(self):
        ...
        self._paused_llm_chunks: List[str] = []  # 暂停期间缓存的LLM文本块
        self._paused_audio_chunks: List[bytes] = []  # 暂停期间缓存的音频块
        self._on_pause_callbacks: List[Callable] = []  # 暂停回调
        self._on_resume_callbacks: List[Callable] = []  # 恢复回调
    ...
    async def _process_audio_internal(self, audio_chunk: bytes) -> Optional[DialogResult]:
        ...
        if self._interrupt_confirm_mode and self._is_streaming:
            ...
            if interrupt_result == "valid":
                ...
                # 非有效人声（咳嗽、嗯等），恢复前端显示和播放
                logger.info(f"[打断] 非有效人声，恢复前端显示/播放")
                await self._resume_frontend()
                ...
                if self._asr_text_buffer.strip() and self._tts_stopped_for_interrupt:
                    ...
                    # 无有效内容，恢复前端显示和播放
                    logger.info("[打断] 超时无有效内容，恢复前端显示/播放")
                    await self._resume_frontend()
    ...
    async def _pause_frontend(self):
        """
        暂停发送给前端（声学VAD检测到用户说话时调用）

        v3.6: 暂停前端显示和播放，但后台任务继续运行
        - 设置暂停标志
        - 通知前端暂停
        """
        if self._is_paused:
        ...
        # 通知前端暂停
        await self._notify_pause()

        logger.info("[暂停] 前端显示/播放已暂停，后台任务继续运行")

    async def _resume_frontend(self):
        """
        恢复发送给前端（语义VAD判断为无效信息时调用）
        ...
        # 发送暂停期间缓存的LLM文本块
        if self._paused_llm_chunks:
            logger.info(f"[恢复] 发送缓存的LLM文本块: {len(self._paused_llm_chunks)} 个")
            for chunk in self._paused_llm_chunks:
                await self._notify_llm_chunk(chunk)
            self._paused_llm_chunks = []
        ...
        # 发送暂停期间缓存的音频块
        if self._paused_audio_chunks:
            logger.info(f"[恢复] 发送缓存的音频块: {len(self._paused_audio_chunks)} 个")
            for audio in self._paused_audio_chunks:
                await self._notify_audio_chunk(audio)
            self._paused_audio_chunks = []
        ...
        logger.info("[恢复] 前端显示/播放已恢复")

    async def _notify_pause(self):
        """通知前端暂停"""
    ...
    async def _process_with_llm(
        ...
        async def on_audio_chunk(audio_data: bytes):
            ...
            if self._should_stop_llm:
                logger.debug(f"[TTS回调] 跳过音频发送: should_stop_llm=True")
                return
            logger.info(f"[TTS回调] 收到音频块: {len(audio_data)} bytes")
            await self._notify_audio_chunk(audio_data)
            logger.info(f"[TTS回调] 音频块已发送到前端")

        # 创建流式TTS处理器并启动消费者任务
        self._streaming_tts = StreamingTTSProcessor(on_audio_chunk=on_audio_chunk)
        logger.info(f"[TTS] 创建StreamingTTSProcessor，准备启动消费者任务...")
        await self._streaming_tts.start()  # v3.4: 启动消费者任务
        logger.info(f"[TTS] StreamingTTSProcessor已启动")
        ...
        def on_llm_chunk(chunk: str):
            ...
            logger.info(f"[LLM回调] 收到文本块: '{chunk[:30]}...' (长度: {len(chunk)})")
            ...
            if self._streaming_tts and not self._streaming_tts._should_stop:
                try:
                    ...
                    logger.info(f"[LLM回调] 文本块已添加到TTS队列: success={success}")
                except asyncio.QueueFull:
                    logger.warning(f"[TTS] 队列已满，跳过文本块: {chunk[:20]}...")
            else:
                logger.warning(f"[LLM回调] TTS处理器不可用: streaming_tts={self._streaming_tts is not None}, should_stop={self._streaming_tts._should_stop if self._streaming_tts else 'N/A'}")
            # 前端显示（创建任务，不阻塞）
    ...
    async def _notify_llm_chunk(self, chunk: str):
        """通知LLM流式输出块"""
        # v3.6: 如果暂停中，缓存数据而不是发送
        if self._is_paused:
            self._paused_llm_chunks.append(chunk)
            logger.debug(f"[暂停] 缓存LLM文本块: {chunk[:20]}...")
            return
    ...
    async def _notify_audio_chunk(self, audio_data: bytes):
        ...
        # v3.6: 如果暂停中，缓存数据而不是发送
        if self._is_paused:
            self._paused_audio_chunks.append(audio_data)
            logger.debug(f"[暂停] 缓存音频块: {len(audio_data)} bytes")
            return

...
- **流式语义判断**：使用 Qwen3-Omni-Flash 边说边判断用户是否说完
...
### 2.5 声学VAD模块 (modules/acoustic_vad.py)
...
class AcousticVAD:
    ...
    silence_threshold_ms = 400  # 静音阈值400ms
    padding_duration_ms = 150   # padding 150ms
```

### 2.6 流式ASR模块 (modules/qwen_asr.py)
...
### 2.7 TTS模块 (modules/tts.py) - v3.2.2 更新
...
│  语义VAD判断人声有效性                                          │
│       │                                                         │
│       ├──── FILLER(语气助词) ──→ 继续播报，继续等待              │
│       │                                                         │
│       ├──── VALID(有效人声) ──→ 立即停止TTS                     │
│       │                        继续接收用户输入                  │
│       │                                                         │
│       └──── COMPLETE(语义完整) ──→ 进入LLM处理                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
